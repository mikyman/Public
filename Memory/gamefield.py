...
class Gamefield(object):
    ...
    def print_board(self):
        ...
        for column in range(self.HEIGHT):
            print(' {}{} {}'.format(' ' if (column+1) <= 9 else '', (column+1), game_board[column]))
    ...
    def choose_field(self, player_name):
        ...
        while(move < 2):
            ...
            if not player_name.endswith('_com'):
                try:
                    print('\n   {},'.format(player_name))
                    inp = input('  please enter e.g. 14 for the right-up-corner or\n  0 for the Mainmenu\n  Choose your field: ')[:2]
                    ...
                    if len(inp) < 2 or inp[-1] == '0' or int(inp) <= 9 or int(inp[0]) > self.HEIGHT or int(inp[1]) > self.LENGTH:
                        print()
                        print('  INVALID MOVE!')
                        print(" Please enter the first number as Height\n und the second number as Length\n e.g. 1{} for the right-up-corner".format(self.LENGTH))
                        invalid = True
